...
// =========================================================
// POST-DATE REVIEW — THE BAD DECISION / ANIMATION V1
...
reviewPanel.addEventListener('change', (event) => {
  ...
  pill.animate([
    ...
    { transform: 'scale(1.08)', boxShadow: '0 0 34px rgba(156,54,93,.42)' },
    { transform: 'scale(1)', boxShadow: '0 0 20px rgba(126,35,70,.20)' }
...
reviewForm.addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    book: window.currentDateConfig?.book?.title || "Amid Clouds and Bones",
    author: window.currentDateConfig?.book?.author || "Ella Fields",
    eventId: window.currentHouseRound?.id || 'house-debut-2026-10',
    eventTitle: window.currentHouseRound?.title || "The House's Debut",
    eventDate: window.currentHouseRound?.date || 'October 2026',
  ...
  try {
    ...
    submitReview.textContent = '♥ FILE THE INCIDENT REPORT ♥';
    reviewStatus.textContent = 'The evidence chute jammed. Please try again.';
  }
});
